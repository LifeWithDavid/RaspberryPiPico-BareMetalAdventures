/* bare metal assembly
 Life with David - BMA Chapter 13_1 - 2nd Core Demo -
 Demonstrates waking up core 1, CPUID, FIFO operation
 Blinks GPIO4 with core 0 and GPIO5 with core 1 
...
// ***********************Main 2 Core Demonstration Routine **************************
// ***********************core 0 blinks GPIO4, Core 1 blinks GPIO5 *******************
.cpu cortex-m0plus
.thumb
...
.equ core1_vt,	0x10000100 		// Core 1 vector table, currently same as core 0 **REVISIT in future**
.equ core1_stack, 0x20041000	// Core 1 stack pointer (can be anywhere, just not core 0 stack)
...
init_core1:	
	...
	adr r4, _entry			// load the entry address for core 1
	...
	cmp r0, r4				// compare the echo 
	bne init_core1		// readback wasn't correct, so start over
	bl delay				// delay to stagger blinking between core 0 and core 1

.align
_entry:					// this is a program that will blink GPIO 4 for core 0 and GPIO 5 for core 1
	ldr r0, =sio_base
	mov r6, #3				// sets bits 0 and 1
	lsl r6, #4				// sets bits 4 and 5
	str r6, [r0, #0x20]		// 0x20 GPIO output enable for GPIO04 and GPIO05
	
_entry_loop:							
	ldr r0, =sio_base		
	ldr r1, [r0, #0]		// load the CPUID register (will return: 0 for core 0, 1 for core 1)
	mov r6, #1				// load 1 into register 6 
	lsl r6, r6, #4			// shift r6 left by 4, (to set up for GPIO4)
	lsl r6, r6, r1			// shift r6 over one more if for core 1 (to set up for GPIO5)
	str r6, [r0, #0x14]		// 0x14 GPIO output atomic value set (turn on GPIO)
	bl delay				// delay so we can see the blinking
	bl delay
	str r6, [r0, #0x18]		// 0x18 GPIO output atomic value clear (turn off GPIO)
	bl delay				// delay so we can see the blinking
	bl delay
	b _entry_loop				// do it again and again
...
fifo_drain_loop:	
	...
	and r1, r2				// use mask to isolate bit 0
...
hex_dump:					// hex dump address starting point in r3
	push {lr}
	mov r4, #32			// number of times to print 128 bytes
hex_dump_1:	
