/* bare metal assembly
 Life with David - BMA Chapter 14_3 - DMA -
 Demonstrates DMA operation by transfering one block of memeory to another
 shows how "number of transfers" carries over to new transfer

 Copyright (C) 2024  David Minderman
...
// releases the peripheral reset for DMA
	ldr r0, =rst_clr	// atomic register for clearing reset controller (0x4000c000+0x3000) 
	mov r1, #1      	// load a 1 into bit 0
	lsl r1, r1, #2		// and shift over 2 bits
	str r1, [r0, #0] 	// store the bitmask into the atomic register to clear register

// check if DMA reset is done
dma_rst:     
    ldr r0, =rst_base	// base address for reset controller
	ldr r1, [r0, #8] 	// offset to get to the reset_done register
	mov r2, #1      	// load a 1 into bit 0
	lsl r2, r2, #2		// and shift over 2 bits
	and r1, r1, r2		// isolate bit 2
	beq dma_rst			// if bit two is 0 then check again, if not, reset is done
...
// ***********************Main DMA Demonstration Routine **************************
// ***********************copies block 1 data into block 2 using words  ************
// *********************** and demonstrates how the number of transfers is maintained**
.cpu cortex-m0plus
.thumb
...
.equ blk_1_start,	0x20010000 		// Start of data block 1
.equ blk_2_start,	0x20020000	    // Start of data block 2
.equ num_transfers,0x040			// number of transfers
.equ dma_config, 	0x001f8039			// DMA Config
.equ dma_base,		0x50000000		// dma base address

// load a pattern into block 1, r0 = blk1 start, r1= blk2 start, r3 = offset

	mov r3, #0				// initialize offset 
_fill_loop:
	ldr r0, =blk_1_start
	ldr r1, =blk_2_start
	mov r5, #01				// load r5 with 01
	strb r3, [r0, r3]		// store an increasing value into block 1
	strb r5, [r1, r3]		// store 01 into block 2
	add r3, r3, #1			// increment offset by 1
	cmp r3, #0xff			// compare with 0x100
	ble _fill_loop			// if the offset is less than 0x100 the go back and do it again
		
// dump the pattern of block 1
	ldr r3, =blk_1_start	// load the start of block 1 into r3 for dumping
	bl hex_dump				// call the hex dump routine 
	bl prt_cr_lf			// print a new line
	
// dump the pattern of block 2
	ldr r3, =blk_2_start	// load the start of block 2 into r3 for dumping
	bl hex_dump				// call the hex dump routine 
	bl prt_cr_lf			// print a new line

// use DMA to move data from block 1 to block 2
	ldr r0, =blk_1_start
	ldr r1, =blk_2_start
	ldr r3, =num_transfers
	ldr r4, =dma_config
	ldr r5, =dma_base
	str r0, [r5, #0]
	ldr r0, [r5, #0]		// load source address register for printing
	bl reg_prt
	bl prt_cr_lf
	str r1, [r5, #4]
	ldr r0, [r5, #4]		// load target address register for printing
	bl reg_prt
	bl prt_cr_lf
	str r3, [r5, #0x08]
	ldr r0, [r5, #0x08]		// load number of transfers register for printing
	bl reg_prt
	bl prt_cr_lf			// print a new line
	str r4, [r5, #12]


	ldr r0, [r5, #8]		// load # of transfers for printing
	bl reg_prt
	bl prt_cr_lf			// print a new line
	ldr r0, [r5, #12]		// load control register for printing
	bl reg_prt
	bl prt_cr_lf			// print a new line
...
// ***** Now do it again, but don't set the number of transfers**********
	mov r3, #0				// initialize offset
_fill_loop_2:
	ldr r0, =blk_1_start
	ldr r1, =blk_2_start
	...
	add r3, r3, #1			// increment offset by 1
	cmp r3, #0xff			// compare with 0x100
	ble _fill_loop_2			// if the offset is less than 0x100 the go back and do it again	
...
//	str r3, [r5, #0x08]
	...
	bl reg_prt
	bl prt_cr_lf			// print a new line
	str r4, [r5, #12]		// stores control and trigger
...
reg_prt:					// print contents of register 0
	push {r0,r1,r4,r5,lr}
...
hex_dump:					// hex dump address starting point in r3
	push {lr}
	mov r4, #2			// number of times to print 128 bytes
hex_dump_1:	
	mov r2, #0			// 8 bytes per line
	mov r1, #0			// r1 number of bytes printed
dump_loop_2:
	bl prt_cr_lf		// print a new line	
	mov r0, r3			// move starting address into r0 for printing
	bl reg_prt			// print the starting address
dump_loop_1:
	bl prt_spc			// print a space
	ldrb r7, [r3,r2]	// load a byte into r7 from memory
	bl prt_byte			// print a byte
	add r2, r2, #1		// increment the address index by 1
	add r1, r1, #1		// increment the number of bytes printed by 1
	cmp r2, #8			// eight bytes printed?
	blt dump_loop_1	// not yet, go back and print another on this line
	add r3, r3, r2		// increment the line starting address by 8
	mov r2, #0			// load 0 into r2 again
	cmp r1, #128		// have 128 bytes been printed yet?
	blt dump_loop_2	// not yet, go back and print another line
	sub r4, r4, #1		// decrement number of 128 dumps to print
	bne hex_dump_1		// if not zero, print another 128 bytes
	pop {pc}			// return
