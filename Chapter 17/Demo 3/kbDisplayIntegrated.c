/* BMA17-Demo 3: This program sends the number of the control key function to an 
   ...
   to the C program using the multi-core FIFO register that then prints out the result to the terminal.
   The assembly program uses an interrupt.  The C program does not.*/
/**
...
// Wait for echo from core0
    uint32_t echo = multicore_fifo_pop_blocking();
    printf("Core1 received echo 0x%08X\n", echo);
...
void kbScanPio(uint pioNum) {
    ...
    // Route the IRQ to Core 1 — must use constants, not variable
    


    // Optional debug print
    
 //   printf("PIO0_IRQ_0 is on core %d\n", irq_get_target_core(PIO0_IRQ_0));
