...
extern uint32_t pendingFunction;   // the pending functions from the assembly program
...
void  core1_FIFO_ISR (void) { //This ISR reads one word from core 0 -> core 1 FIFO"
		
		printf("entering core1_FIFO_ISR, pendingFunction = %04x \n", pendingFunction);
		//multicore_fifo_clear_irq();
		if (multicore_fifo_rvalid()== 1){
			uint32_t core0Data = multicore_fifo_pop_blocking();
			printf("the core0 FIFO word is %08x \n" , core0Data);
			printf("core0Data & 0x80000000 = %08x \n", (core0Data & 0x80000000));
			printf("core1_FIFO_ISR has been cleared, pendingFunction = %04x \n", pendingFunction);
			unsigned char terminalChar = (unsigned char)core0Data;
			printf("Original 32-bit word (hex): 0x%X\n", core0Data);
			printf("8 LSBs as character (hex): 0x%X\n", terminalChar);
			printf("8 LSBs as character (decimal): %d\n", terminalChar);
			printf("8 LSBs as character (ASCII): %c\n", terminalChar); // If it represents a printable ASCII character
		}
		else {
			printf("The FIFO ISR was called, but there was no valid data\n");
		}	
}
   ...
   if ((keyValue & (1<<7))==0) {
	   ...
	   kbReg = kbReg + keyValue;
		...
		switch (kbControl) {
			case 1:
				printf("function 1 \n");
				multicore_fifo_push_blocking(0x80000001); 
				break;
			case 2:
				printf("function 2 \n");
	...
	printf("Made it through switch. pendingFunction = %08x \n", pendingFunction);
...
void main_program(void) {
    stdio_init_all();
//	tusb_init();  //initializes tinyUSB
    sleep_ms(2000);  //gives me time to start PuTTY
	multicore_fifo_drain();
	multicore_fifo_clear_irq();
	printf("Getting ready to set the exclusive core FIFO handler on core %i (C in FLASH) \n",get_core_num());
	// need to write any value to FIFO_ST to clear ROE and WOF flags 2.3.1.4
	irq_set_exclusive_handler(SIO_IRQ_PROC1, core1_FIFO_ISR);
    irq_set_enabled(SIO_IRQ_PROC1, true);
 	printf("kbDisplayIntegrated has taken control, currently running on core %i (C in FLASH) \n",get_core_num());
