...
after_reset: // *************************************after reset*****************
	...
	char x_string[17];
	char *y_disp = malloc(9);  //allocate 17 bytes for character array (16 for word and one for end byte)
	char y_string[17];
	char *p_disp = malloc(9);  //allocate 17 bytes for character array (16 for word and one for end byte)
	char p_string[17];
	char run[] =" RUN";
	char halt[] = "HALT";
